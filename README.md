...
## GitHub Actions lock-file note

This prototype intentionally does not require `package-lock.json`. The Pages workflow uses `npm install` and does not enable `actions/setup-node` npm caching, because that cache mode requires one of `package-lock.json`, `npm-shrinkwrap.json`, or `yarn.lock` at the repository root.

If you later run `npm install` locally, commit the generated `package-lock.json` and you may switch the workflow install step to `npm ci`.
