...
const SAME_SIDE_INSET = 170;
const MIN_HANDLE_CLEARANCE = 56;
...
function compareConnection(a: SpliceConnection, b: SpliceConnection): number {
  return (
    a.source.cableName.localeCompare(b.source.cableName) ||
    String(a.source.tubeColor).localeCompare(String(b.source.tubeColor)) ||
    a.source.fiberNumber - b.source.fiberNumber ||
    a.target.cableName.localeCompare(b.target.cableName) ||
    String(a.target.tubeColor).localeCompare(String(b.target.tubeColor)) ||
    a.target.fiberNumber - b.target.fiberNumber ||
...
function pointsToSegments(points: Point[]): Segment[] {
  ...
  for (let i = 1; i < points.length; i += 1) {
    ...
    segments.push({
      kind: Math.abs(from.y - to.y) < 0.01 ? "h" : "v",
      from,
      to,
    });
  }
...
function horizontalDistanceOk(source: Point, target: Point, midX: number): boolean {
  return Math.abs(midX - source.x) >= MIN_HANDLE_CLEARANCE && Math.abs(midX - target.x) >= MIN_HANDLE_CLEARANCE;
}

function midpointLaneX(args: {
  source: Point;
  target: Point;
  lane: number;
  laneCount: number;
  ...
  const spread = (lane - (laneCount - 1) / 2) * LANE_SPACING;
...
function validateRoutes(routes: RoutedStrand[]): RoutingDiagnostic[] {
  ...
  for (let i = 0; i < routes.length; i += 1) {
    for (let j = i + 1; j < routes.length; j += 1) {
      ...
      const verticalA = a.segments.find((seg) => seg.kind === "v");
      const verticalB = b.segments.find((seg) => seg.kind === "v");
      if (!verticalA || !verticalB) continue;
      const sameTrack = Math.abs(verticalA.from.x - verticalB.from.x) < LANE_SPACING - 0.5;
      if (sameTrack && verticalRangesOverlap(verticalA, verticalB)) {
        diagnostics.push({
          level: "warning",
          code: "ROUTE_VERTICAL_STACK",
          message: `Routes ${a.connectionId} and ${b.connectionId} use nearly the same vertical lane.`,
...
export function routeSpliceStrands(
  ...
  for (const conn of model.connections) {
    ...
    if (!anchors) {
      diagnostics.push({
        level: "error",
        code: "MISSING_ANCHOR",
        message: `Missing source or target anchor for ${conn.id}.`,
  ...
  for (const [key, connections] of groups) {
    ...
    for (const [lane, conn] of ordered.entries()) {
      ...
      if (!horizontalDistanceOk(source, target, midX)) {
      ...
      routes.push({
        ...
        lane,
        zoneKey: key,
        color: fiberColorHex[conn.source.fiberColor],
        protected: Boolean(overrides.protectedConnectionIds[conn.id]),
